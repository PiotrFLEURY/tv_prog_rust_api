...
pub async fn init_xml_tv_data() -> Result<(), String> {
    ...
    postgres_client::drop_channels().await?;
    println!("Existing channels dropped from the database.");

    postgres_client::drop_programs().await?;
    ...
    postgres_client::save_channels(channels.clone()).await?;
    postgres_client::save_channel_packages(channels.clone(), "ALL".to_string()).await?;
    ...
    for chunk in program_chunks {
        postgres_client::bulk_insert_programs(chunk.to_vec()).await?;
    ...
    postgres_client::save_channel_packages(known_fr_channels, "FR".to_string()).await?;
    ...
    postgres_client::save_channels(unknown_fr_channels.clone()).await?;
    postgres_client::save_channel_packages(unknown_fr_channels, "FR".to_string()).await?;
    let unknown_fr_programs = fr
        .programs
        .into_iter()
        .filter(|p| !existing_channel_ids.contains(&p.channel))
        .map(program_converter::model_to_entity)
        .collect::<Result<Vec<Program>, String>>()?;
    if !unknown_fr_programs.is_empty() {
        println!("Found {} unknown FR channels", unknown_fr_programs.len());
        postgres_client::bulk_insert_programs(unknown_fr_programs).await?;
    } else {
    ...
    postgres_client::save_channel_packages(known_tnt_channels, "TNT".to_string()).await?;
